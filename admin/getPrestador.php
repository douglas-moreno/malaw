<?php
    ...
    $sql = "Select Codigo_Prestador, Prestador, Ativo FROM Prestador order by Prestador";
    $DB = new mysql();
    ...
    <p align='center'><a href='addPrestador.php' class='button round tiny'>Adicionar</a></p>
    ...
    while ($linha = $DB->FetchArray($querytipo)) {
        $tabela.= "<tr><td>". $linha["Prestador"]. "</td>";
        if($linha["Ativo"] == 1){
            $tabela.= "<td><i class='fi-check'></i></td>";
            $tabela .= "<td><a onClick=\"return confirm('Confirma Desativar Prestador?')\" href='prestadorremovido.php?id=" . $linha["Codigo_Prestador"] . "&ativo=" . $linha["Ativo"] . "'><i class='fi-loop'></i></a></td></tr>";
        }
        else{
            $tabela.= "<td><i class='fi-x'></i></td>";
            $tabela .= "<td><a onClick=\"return confirm('Confirma Ativar Prestador?')\" href='prestadorremovido.php?id=" . $linha["Codigo_Prestador"] . "&ativo=" . $linha["Ativo"] . "'><i class='fi-loop'></i></a></td></tr>";
        }
    }
    $tabela.= "</tbody></table>";
    $DB->Close();
...
<html class="no-js" lang="en" > 
    ...
    <body>
        ...
        <dl class="sub-nav">
            ...
            <dd class="active"><a href="getPrestador.php">Prestador</a></dd>
        </dl>

        <div class='row'>
            <div class='small-2 small-centered columns'>
                <h2 align='center'>Prestador</h2>
            </div>
        </div>
