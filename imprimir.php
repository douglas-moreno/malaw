...
class PDF extends FPDF {
    ...
    $sqlbase = baseCondicao("papel", $eten, $dia, $niver, $dia2, $niver2, $cepc, $nome, $regiao, $bairro, $prestador, $tipos, $filho, $ordem, $numeros, $cadastro1, $cadastro2);
    if($eten=="1") {
        $pdf = new PDF("P","mm","A4");
        $pdf->Open();
        $pdf->AddPage();
    ...
    function etiqueta($pdf, $base){
        ...
        while($dados = $DB->FetchArray($queryet)) {
            ...
            if(strlen($ende) > 40) {
                $posicao = strpos(mb_substr($ende,40),' ') + 40;
                $local = mb_substr($ende,$posicao) . ", " . $dados["Num"] . "-";
